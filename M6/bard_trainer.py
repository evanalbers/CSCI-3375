...
def feedforward(X, synapse_0, synapse_1):
    ...
    layer_0 = X
    layer_1 = db.sigmoid(np.dot(layer_0, synapse_0))
    layer_2 = db.sigmoid(np.dot(layer_1, synapse_1))
...
def get_synapses(epochs, X, y, alpha, synapse_0, synapse_1):
    ...
    for j in iter(range(epochs+1)):
        ...
        layer_2_delta = layer_2_error * db.sigmoid_output_to_derivative(layer_2)
        ...
        layer_1_delta = layer_1_error * db.sigmoid_output_to_derivative(layer_1)
...
def bow(sentence, words):
    ...
    sentence_words = [stemmer.stem(word.lower()) for word in word_tokenize(sentence)]
...
def get_output_layer(words, sentence):
    ...
    l1 = db.sigmoid(np.dot(l0, synapse_0))
    # Create the output layer.
    l2 = db.sigmoid(np.dot(l1, synapse_1))
    return l2
